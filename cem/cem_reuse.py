...
class CEMPlanner:
    ...
    def plan(self, env, save_state, restore_state):
        ...
        if not self.warm_starts:
            ...
            self.action_dist.lower = np.load('/lab/ssontakk/pybullet_fingers/cem_planning/lower_travel_1.npy')
            self.action_dist.upper = np.load('/lab/ssontakk/pybullet_fingers/cem_planning/upper_travel_1.npy')
            print(self.action_dist.upper)
            print(self.action_dist.lower)
            print('warm started')
        ...
        for i_iteration in maybe_progbar(range(self.n_iterations)):
            ...
            # np.save(best_return, '/lab/ssontakk/pybullet_fingers/cem_planning/lower.npy')
        ...
        restore_state(initial_state)
        _best_returns = np.array(_best_returns)
        _worst_returns = np.array(_worst_returns)

        np.save(f'Travelybest_Warm{self.warm_starts}.npy', _best_returns)
        np.save(f'Travelyworst_Warm{self.warm_starts}.npy', _worst_returns)
